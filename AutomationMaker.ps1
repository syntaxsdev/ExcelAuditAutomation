...
$excelScripts.newAutomation("Autofill", {
    ...
    #if the file is no data, the file is already moved to complete. exit the function
    if ($excel.GetCellValue("A2") -eq 'No Data') {
        $excel.SaveQuitAndMove("{completed}")
        return
    }
...
$excelScripts.newAutomation("DailySql-MoveAll", {
    $excel.SaveCopyAndQuit("{}")
})
